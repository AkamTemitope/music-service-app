...
export const createOrRetrieveCustomer = async ({ email, uuid }: createOrRetrieveCustomerProps ) => {
...
export const manageSubscriptionStatusChange = async ( subscriptionId: string, customerId: string, createAction = false ) => {
  ...
  const { data: customerData, error: noCustomerError } = await supabaseAdmin.from('customers').select('id')
